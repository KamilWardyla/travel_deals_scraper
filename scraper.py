...
class WebScraper(DataBaseUtils):
    def __init__(self, ):
        super().__init__()
        self.destination_list = ["Dominikana", "Lanka", "Meksyk", "Zanzibar", "Tanzania", " Malediwy",
                                 "Wyspy Zielonego przyladka", "Kuba", "Aruba", "Bali", "Indonezja", "Tajlandia",
                                 "Bangkok", "Wietnam", "Fuertaventura", "Phuket", "Krabi", "Phu Quoc", "Mombasa",
                                 "Kenia", "Mauritius", "Varadero", "Majowka", "Czarnogóra", "Turcja"]
    ...
    def web_scraper_last_minuter(self):
        """
            ...
            Retrieves information about last-minute holiday deals from the website, compares
            ...
            Returns:
            - str: A string indicating the status of the lastminuter job along with a timestamp,
                    ...
                    for destination in self.destination_list:
                        for word in text.split():
                            if self.similar(destination.lower(), word.lower()) > 0.8:
                                if (text, link) not in self.get_data_from_table_holiday():
    ...
    def web_scraper_wakacyjni_piraci(self):
        ...
        if response.status_code == 200:
            ...
            print(
                f"{datetime.datetime.now()} --- Error::Response status code != 200: {response.status_code}")
        ...
        for destination in self.fly_dict.keys():
            for url in self.fly_dict[destination]:
                ...
                if r_result:
                    ...
                    if (destination, date_str) not in self.get_date_from_table_fly():
                    ...
                    if (
                            destination,
                            date_str) in self.get_date_from_table_fly() and price != self.get_price_from_table_fly(
                        destination=destination,
                        date=date_str) and price <= self.price_dict[destination]:
                        self.update_price_in_fly_table(destination=destination, date=date_str, new_price=price)
